...
q_doc_eu_lake <- quantile(cnp_eu_lake$log_DOC_med, c(0.5))
q_doc_eu_river <- quantile(cnp_eu_river$log_DOC_med, c(0.5))
q_doc_na_lake <- quantile(cnp_na_lake$log_DOC_med, c(0.5))
q_doc_na_river <- quantile(cnp_na_river$log_DOC_med, c(0.5))

# Phos quantiles to use
q_phos_eu_lake <- quantile(cnp_eu_lake$log_phos_med, c(0.5))
q_phos_eu_river <- quantile(cnp_eu_river$log_phos_med, c(0.5))
q_phos_na_lake <- quantile(cnp_na_lake$log_phos_med, c(0.5))
q_phos_na_river <- quantile(cnp_na_river$log_phos_med, c(0.5))
...
pred_svc_na_doc_lake <- predict_SVC(svc_eu_lake, Xp = as.matrix(svc_data_eu_lake_doc))
...
pred_svc_na_phos_lake <- predict_SVC(svc_eu_lake, Xp = as.matrix(svc_data_eu_lake_phos))
...
```{r}
pred_doc %>%
  ggplot(aes(x = log_DOC_med, y = fit_ols, group = ECO_TYPE)) +
