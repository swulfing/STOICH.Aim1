...
& + \beta_1 (\log_{10}DOC) + \beta_1(\textbf{s}_i) (\log_{10}DOC) \\
& + \beta_2 (\log_{10}PO4) + \beta_2(\textbf{s}_i) (\log_{10}PO4) \\
& + \beta_3 (\log_{10}DOC)\times (\log_{10}PO4) + \epsilon_i
\end{align*}

Where $y_i$ is the $log_{10}$ transformed annual median NO3 for site $i$, and the $\beta_{j}(\textbf{s}_i)$ is the varying slope coefficients (intercept, DOC, PO4) that has distribution $GP(0, \Sigma_j)$ where $\Sigma_j$ has an exponential correlation function (need to check variograms to see if exponential is reasonable.) The nugget (residual error) is modeled by $\epsilon_i \sim N(0, \tau^2)$.
...
#### Make SVC maps for na rivers

```{r}
plot_svc_sites(svc_na_lake)
```
...
ggplot() + 
    ...
    geom_map(aes(map_id = region), col = "gray", fill = NA, 
           data = world_map, map = world_map) +
    labs(title = "Spatial variation of intercept", x = "Longitude", y = "Latitude"
...
ggplot() + 
    ...
    geom_map(aes(map_id = region), col = "gray", fill = NA, 
           data = world_map, map = world_map) +
    labs(title = "Uncertainty of spatially varying intercept", x = "Longitude", y = "Latitude"
...
ggplot() + 
    ...
    geom_map(aes(map_id = region), col = "gray", fill = NA, 
           data = world_map, map = world_map) +
    labs(title = "Spatial variation of slope of log(DOC)", x = "Longitude", y = "Latitude"
...
ggplot() + 
    ...
    geom_map(aes(map_id = region), col = "gray", fill = NA, 
           data = world_map, map = world_map) +
    labs(title = "Uncertainty of spatially varying slope of log(DOC)", x = "Longitude", y = "Latitude"
         ,fill = expression(sd(widehat(beta[1](s))))
         ) +
    scale_fill_viridis()
...
ggplot() + 
    ...
    geom_map(aes(map_id = region), col = "gray", fill = NA, 
           data = world_map, map = world_map) +
    labs(title = "Spatial variation of slope of log(TP)", x = "Longitude", y = "Latitude"
...
ggplot() + 
    ...
    geom_map(aes(map_id = region), col = "gray", fill = NA, 
           data = world_map, map = world_map) +
    labs(title = "Uncertainty of spatially varying slope of log(PO4)", x = "Longitude", y = "Latitude"
...
The fitted vs. residuals plots show some sign of bias for NA and EU lakes where the model overestimates low NO3 sites and underestimates high NO3 sites.

From the QQ-plots we can see that the model residuals are heavy-tailed. This is probably the consequence of the model bias mentioned previously. 
